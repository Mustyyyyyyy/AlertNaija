...
export default function LiveMapPage() {
  ...
  /* ── Mapbox GL JS initialisation (lazy-load) ── */
  useEffect(() => {
    ...
    const load = async () => {
      ...
      // GeolocateBtn — button only; we handle flyTo ourselves via useUserLocation
  ...
  /* ── Fly to user location the moment it resolves ── */
  useEffect(() => {
  ...
  /* ── Manual "Locate Me" button — fires getCurrentPosition on demand ── */
  ...
  /* ── Compose LatLng string for the locateMe label ── */
    ...
    <main className="min-h-screen bg-background-dark flex flex-col">
      ...
      <header className="sticky top-0 z-50 w-full bg-black/60 backdrop-blur-xl border-b border-white/5">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-10 h-14 flex items-center justify-between">
          <h1 className="text-lg font-heading font-bold text-white">
            ...
            <span className="text-slate-500 text-sm font-body font-normal">· Live Map</span>
          </h1>
      ...
      <div className="flex-1 relative">
        ...
        >
          ...
          <span>
            {locStatus === "acquiring" ? "Locating…" : coordsDisplay ? "Recenter on me" : "Find my location"}
          </span>
        </button>
      ...
      >
        ...
        {incident.responder && (
          <div className="bg-primary/5 border border-primary/20 rounded-xl px-4 py-3 flex items-center gap-2">
            ...
            <span className="text-slate-500 text-xs">· {incident.responder.type}</span>
          </div>
        )}
      </div>
    </div>
