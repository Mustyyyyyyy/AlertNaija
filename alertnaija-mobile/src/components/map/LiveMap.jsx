 ...
 *
 * Props
 *  userLocation â€” "lat, lng" string | null
 *  incidents    â€” optional override; falls back to Zustand store
 */
  ...
  return (
    <div className="bg-gradient-to-b from-[#111826] to-[#0a0f1a] border border-slate-800 rounded-2xl overflow-hidden h-[420px] relative">
      ...
      {markers.map((inc) => {
        ...
        return (
          <div
            ...
            title={`${inc.type} â€” ${inc.user?.state || "unknown"} â€” ${inc.status}`}
          >
            {/* pulse ring */}
      ...
      <div className="absolute bottom-4 right-4 z-10 space-y-1 text-right">
        {markers.length === 0 && (
          <p className="text-slate-600 text-xs">{userLocation ? "No active incidents" : "Waiting for user locationâ€¦"}</p>
        )}
        {markers.length > 0 && (
          <div className="bg-black/50 backdrop-blur rounded-lg px-3 py-2 space-y-1 text-[11px]">
            {["FIRE","MEDICAL","SECURITY","RESCUE"].map((t) => {
              ...
              return <p key={t} className={color}>{t} Â· {count}</p>;
            })}
          </div>
        )}
      </div>
